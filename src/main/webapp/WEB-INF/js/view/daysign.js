...
function createTable(){
    ...
    table = $('#userTable').DataTable( {
        ...
        columns: [
            ...
            {"data":"employeeByEmployeeId.phone"},
        ...
        columnDefs: [{
            ...
            render: function (data, type, row,   meta ) {
                ...
                var html = "";
                if(row.state == 0){
                    html = "<label class='text-success'>已签到</label>"
                }
                if(row.state == 1){
                    html = "<label style='color: #d58512'>迟到</label>"
                }
                if(row.state == 2){
                    html = "<label class='text-warning'>请假</label>"
                }
                if(row.state == 3){
                    html = "<label class='text-danger'>旷工</label>"
                }
                if(row.state == 9999){
                    html = "<label >未签到</label>"
                }
...
function editstate(state){
    ...
    var a = $("#timestart").val().split("-");
    str += "&editinfo=" + JSON.stringify(employees)+"&state="+state+"&year="+a[0]+"&month="+a[1];
