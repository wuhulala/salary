...
@Repository
public class YearSalaryDaoImpl extends BaseDaoImpl<YearSalary, Integer> implements YearSalaryDao {

    public YearSalary findByEmployeeId(int id) {
        return (YearSalary) getSession().createCriteria(YearSalary.class)
    ...
    public Map findByParams(String searchInfo, int year, int department, int position, int start, int length) {
        Criteria criteria = getSession().createCriteria(YearSalary.class);
